...
class EDAGPT:
    ...
    def messagesinterface(self):
        def checkgraphpresence(tupledata):
            return any(isinstance(ele, str) and "graphed" in ele.lower() for ele in tupledata)
        ...
        for message in st.session_state.chat_history:
            with st.chat_message(message['role']):
                if message['role'] == "user":
                    ...
                    elif isinstance(metadatas, tuple) and checkgraphpresence(metadatas):
                        ...
                        try:
                            ...
                            if plot_files:
                                for index, file in enumerate(plot_files):
                                    st.image(os.path.join(plots_path, file), caption=f"Graph {index+1}")
                            else:
                                st.warning("No plots found in plot directory.")
                        except Exception as e:
                            st.error(f"Error loading plots: {e}")

                        # Show any extra output (besides 'graphed') and code
                        for metadata in metadatas:
                            if isinstance(metadata, str) and metadata.lower() != "graphed":
                                st.write(metadata)
                        if response:
                            st.code(response)

                    elif not isinstance(metadatas, tuple) and metadatas.lower() == "try again":
